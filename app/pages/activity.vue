<template>
    <div>
        ...
        <div style="margin-top: 0.1rem; margin-bottom: 4rem;">
            <div 
            ...
            >
                ...
                >
                    ...
                    <!-- ใช้ span สำหรับจัดตำแหน่ง v-select -->
                    <span style="position: absolute; right: 1rem;">
                        <v-select
                            v-model="student.selectedValue"
                            :items="['มา', 'ลา / สาย', 'ขาด']"
                            style="width: 95px; margin-left: auto;"
                            hide-details
                            ...
                            dense
                            :style="{
                                backgroundColor: student.selectedValue === 'มา' ? '#fafafa' : 
                                                student.selectedValue === 'ลา / สาย' ? '#ffface' : 
                                                student.selectedValue === 'ขาด' ? '#ffd3d3' : 'aliceblue'
                            }"
                        />
                    </span>
                </v-btn>
        ...
        <!-- Dialog for Save Activity Success or Failure -->
        <v-dialog v-model="dialogSaveSuccess" max-width="400px">
            <v-card>
                <v-card-title>
                    <span>ผลการบันทึกข้อมูล</span>
                </v-card-title>
                <v-card-text>
                    <div>{{ saveMessage }}</div>
                </v-card-text>
                <v-card-actions>
                    <v-btn color="blue" text @click="dialogSaveSuccess = false">ปิด</v-btn>
                </v-card-actions>
            </v-card>
        </v-dialog>
    </div>
...
const dialogSaveSuccess = ref(false); // เปิด/ปิด dialog สำหรับผลการบันทึก
const saveMessage = ref(''); // ข้อความสำหรับแสดงใน dialog
...
const uploadImage = async () => {
    ...
    try {
        ...
        saveMessage.value = 'อัพโหลดรูปภาพสำเร็จ!';
        dialogSaveSuccess.value = true;

        // โหลดข้อมูลใหม่เพื่ออัปเดต UI
        loadData();
    } catch (error) {
        console.error('เกิดข้อผิดพลาดในการอัพโหลด', error);
        saveMessage.value = 'เกิดข้อผิดพลาดในการอัพโหลดรูปภาพ';
        dialogSaveSuccess.value = true;
    }
};
...
const loadData = async () => {
    try {
        ...
        const getImgID = await axios.get(`/api/getImgID?dateID=${dateID}&groupID=${groupID}`);
    ...
    try {
        ...
        list.value = studentsResponse.data.map(student => ({
            ...
            selectedValue: attendanceMap[student.stdID] !== undefined
                ? (attendanceMap[student.stdID] === 1 ? 'มา' : attendanceMap[student.stdID] === 0.5 ? 'ลา / สาย' : 'ขาด')
                : 'มา'
        }));
...
const saveActivity = async () => {
    const studentData = list.value.map(student => {
        let checkStatus = 1; // ค่าเริ่มต้น
        if (student.selectedValue === 'มา') checkStatus = 1;
        if (student.selectedValue === 'ลา / สาย') checkStatus = 0.5;
        if (student.selectedValue === 'ขาด') checkStatus = 0;
    ...
    try {
        ...
        saveMessage.value = 'บันทึกข้อมูลสำเร็จ!'; // ตั้งข้อความเมื่อบันทึกสำเร็จ
        dialogSaveSuccess.value = true; // เปิด dialog แจ้งเตือน
    } catch (error) {
        console.error('Error saving activity:', error);
        saveMessage.value = 'เกิดข้อผิดพลาดในการบันทึกข้อมูล'; // ข้อความเมื่อเกิดข้อผิดพลาด
        dialogSaveSuccess.value = true; // เปิด dialog แจ้งเตือน
    }
};
